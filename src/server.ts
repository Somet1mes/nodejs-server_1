...
import Status_Effect from './Game/Player/Status_Effect'
...
let app = express();
import imp_server = require('http');
let server = new imp_server.Server(app);
import imp_io = require('socket.io');
let io = imp_io.listen(server);
...
var game_1 = new Game(period, mapPath, setPath);
    ...
    socket.on('c_t_s_chat_msg',
        function(msg)
        {
            if (socket.checked === true)
            {
                ...
                console.log(msg);
                let msg_out = {name:socket.playerName, text:msg}; // stops the client sending a fake name
