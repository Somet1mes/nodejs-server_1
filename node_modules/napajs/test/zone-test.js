...
describe('napajs/zone', function () {
    ...
    describe('broadcast', function () {
        ...
        it.skip('@node: -> node zone with anonymous function having closure (should fail)', function () {
            return shouldFail(function () {
    ...
    describe('broadcastSync', function () {
        it('@node: -> napa zone with JavaScript code', function () {
            napaZone1.broadcastSync("var state = 0;");
        });
        it('@napa: -> napa zone with JavaScript code in different zone', function () {
            return napaZone1.execute('./napa-zone/test', "broadcastSync", ["napa-zone2", "var state = 0;"]);
        });
        it('@napa: -> napa zone with JavaScript code in current zone (should fail)', function () {
            return shouldFail(function () {
                return napaZone1.execute('./napa-zone/test', "broadcastSync", ["napa-zone1", "var state = 0;"]);
            });
        });
        it('@node: -> napa zone with anonymous function', function () {
            napaZone1.broadcastSync(function (input) {
                console.log(input);
            }, ['hello world']);
        });
        it('@napa: -> napa zone with anonymous function', function () {
    ...
    describe('execute', function () {
        ...
        var unicodeStr = "中文 español deutsch English हिन्दी العربية português বাংলা русский 日本語 ਪੰਜਾਬੀ 한국어 தமிழ் עברית"; // len = 92
        it('@node: -> node zone with args containing unicode string', function () {
            return napa.zone.current.execute(function (str) {
                var assert = require("assert");
                ...
                return str;
            }, [unicodeStr]).then(function (result) {
                assert.equal(result.value, unicodeStr);
            });
        });
        it('@node: -> napa zone with args containing unicode string', function () {
            return napaZone1.execute(function (str) {
                var assert = require("assert");
                var unicodeStr = "中文 español deutsch English हिन्दी العربية português বাংলা русский 日本語 ਪੰਜਾਬੀ 한국어 தமிழ் עברית";
                assert.equal(str, unicodeStr);
                return unicodeStr;
        ...
        it('@napa: -> napa zone with args containing unicode string', function () {
            return napaZone1.execute('./napa-zone/test', "executeWithArgsContainingUnicodeString", ['napa-zone2'])
                .then(function (result) {
                assert.equal(result.value, unicodeStr);
            });
        });
        it('@napa: -> node zone with args containing unicode string', function () {
